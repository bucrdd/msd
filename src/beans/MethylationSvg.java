...
public class MethylationSvg {

	private List<SiteMeth> list;
	private int start;
	private int end;
	...
	private Element getSiteMethElement(Document document){
		...
		for(int i=0;i<list.size();i++){
			...
			rect.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "y", ""+(height*(1-list.get(i).getMethyValue())));
			rect.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "width", ""+(widthMin>0.5?widthMin:0.5));
			rect.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "height", ""+(height*list.get(i).getMethyValue()));
			g.appendChild(rect);
		}
		return g;
	}
	
	public void createSvg(List<SiteMeth> list,int start,int end,File file) throws IOException{
...
//				"alert(pisition.value);\n"+
	...
	public List<SiteMeth> getList() {
		return list;
	}

	public void setList(List<SiteMeth> list) {
		this.list = list;
	}
