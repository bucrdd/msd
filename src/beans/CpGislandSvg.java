...
public class CpGislandSvg {

	private List<CpGisLand> list;
	private int start;
	private int end;
...
//			a.setAttribute("xlink:href", "genomeBrowseViewPage.action?position="+list.get(i).getChrom()+":"+list.get(i).getChromStart()+"-"+list.get(i).getChromEnd());
	...
	public void createSvg(List<CpGisLand> list,int start,int end,File file) throws IOException{
...
//				"alert(pisition.value);\n"+
	...
	public List<CpGisLand> getList() {
		return list;
	}

	public void setList(List<CpGisLand> list) {
		this.list = list;
	}
