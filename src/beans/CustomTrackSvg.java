...
public class CustomTrackSvg {

	private List<CustomTrack> list;
	private int start;
	private int end;
	private String color=Color.grey;
	
	private Element getCustomTrackElement(Document document){
...
//			a.setAttribute("xlink:href", "genomeBrowseViewPage.action?position="+list.get(i).getChrom()+":"+list.get(i).getChromStart()+"-"+list.get(i).getChromEnd());
			...
			rect.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "width", ""+(Math.abs(list.get(i).getEnd()-list.get(i).getStart())*widthMin>0.5?Math.abs(list.get(i).getEnd()-list.get(i).getStart())*widthMin:0.5));
	...
	//		rect.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "stroke-opacity",getOpacity()+"");
			...
			rect.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "title", ""+(list.get(i).getScore()));
	...
	private Element getCustomTrackHistElement(Document document){
		...
		for(int i=0;i<list.size();i++){
			...
			rect.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "fill", color);
			rect.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "x", ""+(list.get(i).getStart()-start)*widthMin);
			rect.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "y", ""+(height*(1-Double.parseDouble(list.get(i).getScore()))));
			rect.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "width", ""+(widthMin>0.5?widthMin:0.5));
			rect.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "height", ""+(height*(Double.parseDouble(list.get(i).getScore()))));
			g.appendChild(rect);
		}
		return g;
	}
	
	public void createSvg(List<CustomTrack> list,int start,int end,File file,String type) throws IOException{
...
//				"alert(pisition.value);\n"+
		...
		if(type.equals("region")){
			el=this.getCustomTrackElement(document);
		}else{
			el=this.getCustomTrackHistElement(document);
		}
	...
	public List<CustomTrack> getList() {
		return list;
	}

	public void setList(List<CustomTrack> list) {
		this.list = list;
	}
