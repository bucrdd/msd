...
public class SnpSvg {
	...
	private List<SNP> list;
	private int start;
	private int end;
	private String color=Color.brown;
	
	private Element getSnpElement(Document document){
...
//			a.setAttribute("xlink:href", "genomeBrowseViewPage.action?position="+list.get(i).getChrom()+":"+list.get(i).getChromStart()+"-"+list.get(i).getChromEnd());
	...
	//		rect.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "stroke-opacity",getOpacity()+"");
			...
			rect.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "title", ""+(list.get(i).getName()+" -- "+list.get(i).getDisease()));
			
			Element element=document.createElementNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "text");
			double space = 0;
			if (i==0){
				space = (list.get(i).getStart()-start)*widthMin;
			}else{
				space = (list.get(i).getStart()-list.get(i-1).getEnd())*widthMin;
			}
			if (space >= (list.get(i).getName().length()*6+10)){
				...
				element.setAttributeNS(SVGDOMImplementation.SVG_NAMESPACE_URI, "x", ""+((list.get(i).getStart()-start)*widthMin-list.get(i).getName().length()*6-10));
	...
	public void createSvg(List<SNP> list,int start,int end,File file) throws IOException{
...
//				"alert(pisition.value);\n"+
		...
		"position.Value=\"chr1:100-1000\";"+
					"alert(go);\n"+
		"}"));
		Element el=this.getSnpElement(document);
	...
	public List<SNP> getList() {
		return list;
	}

	public void setList(List<SNP> list) {
		this.list = list;
	}
